...
const VideoPromptStepWithTemplates = ({
  ...
  const handleGeneratePromptsFromChatGPT = async () => {
    ...
    try {
      // Try to use language-aware prompt generation
      const analysis = {
        character: {},
        product: {}
      };
      ...
      try {
        ...
        if (response?.success || response?.positive) {
          // Use generated prompt as template for all segments
          const basePrompt = response.positive || response.data?.positive || '';
          const defaultPrompts = Array(segments)
            .fill('')
            .map((_, idx) => `${idx + 1}/${segments}: ${basePrompt}\n\n[Segment-specific details for this ${maxPerVideo}s part...]`);
          
          onPromptsChange(defaultPrompts);
          return;
        }
      } catch (apiError) {
        console.warn('Language-aware prompt generation failed, using template fallback', apiError);
      }

      // Fallback: Use default template prompts with segment numbering
      const defaultPrompts = Array(segments)
        .fill('')
        .map((_, idx) => `Segment ${idx + 1}/${segments}: [Generated by AI]\nDescribe the action, camera movement, product focus, and details for this ~${maxPerVideo}s segment. Include transitions, lighting, and any special effects.`);
      
      onPromptsChange(defaultPrompts);
      setPromptError(null);
    } catch (error) {
      console.error('Error generating prompts:', error);
      setPromptError('Could not generate enhanced prompts. Using template instead.');
      handleAutoFill();
    } finally {
