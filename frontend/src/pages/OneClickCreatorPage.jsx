...
export default function OneClickCreatorPage() {
  ...
  const startPreviewPolling = (flowId, sessionId, stopRef = { stop: false }) => {
    ...
    const MAX_POLLS = 300;
    const IDLE_TIMEOUT_MS = 3 * 60 * 1000; // 3 minutes without new preview data
    const DB_STATUS_CHECK_INTERVAL_MS = 10000; // query DB every 10s

    let pollCount = 0;
    let lastPreviewUpdatedAt = 0;
    let lastDataActivityAt = Date.now();
    let lastDbCheckAt = 0;

    const applyPreviewToSession = (preview) => {
      setSessions(prev => prev.map(s => {
        ...
        if (preview.status === 'failed' || preview.status === 'error') {
          nextSession.completed = false;
    ...
    const checkDbSessionStatus = async () => {
      try {
        const res = await fetch(`/api/ai/affiliate-video-tiktok/session-status/${flowId}`);
    ...
    const pollPreview = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Track activity based on preview updatedAt to detect stale polling
          if (preview.updatedAt && preview.updatedAt !== lastPreviewUpdatedAt) {
            lastPreviewUpdatedAt = preview.updatedAt;
            lastDataActivityAt = Date.now();
          }

          applyPreviewToSession(preview);

          if (preview.status === 'completed' || preview.status === 'failed' || preview.status === 'error' || stopRef.stop) {
            return true;
          }
        }

        // Auto-stop if no new data for 3 minutes
        if (Date.now() - lastDataActivityAt >= IDLE_TIMEOUT_MS) {
          addLog(sessionId, '⏱️ Polling stopped: no new preview data for 3 minutes');
          setSessions(prev => prev.map(sess => sess.id === sessionId ? { ...sess, error: sess.error || 'Preview timeout (no new data in 3 minutes)' } : sess));
          return true;
        }

        // Query DB status periodically to stop polling when session is done/failed
        if (Date.now() - lastDbCheckAt >= DB_STATUS_CHECK_INTERVAL_MS) {
          lastDbCheckAt = Date.now();
          const dbStatus = await checkDbSessionStatus();

          if (dbStatus === 'completed') {
            addLog(sessionId, '✅ Polling stopped: session status is completed in DB');
            setSessions(prev => prev.map(sess => sess.id === sessionId ? { ...sess, completed: true, error: null } : sess));
            return true;
          }

          if (dbStatus === 'failed') {
            addLog(sessionId, '❌ Polling stopped: session status is failed in DB');
            setSessions(prev => prev.map(sess => sess.id === sessionId ? { ...sess, completed: false, error: sess.error || 'Session failed (DB status)' } : sess));
            return true;
          }
        }
        ...
        // Keep polling unless explicitly stopped, timeout will still apply
        return stopRef.stop;
      }
    };
    ...
    const pollInterval = setInterval(async () => {
      ...
      if (shouldStop || pollCount >= MAX_POLLS || stopRef.stop) {
