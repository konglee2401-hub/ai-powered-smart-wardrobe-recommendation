  ...
  async uploadImages(characterImagePath, productImagePath, existingImages = []) {
    ...
    try {
      ...
      for (let fileIdx = 0; fileIdx < filesToProcess.length; fileIdx++) {
        ...
        // Capture gallery count before this upload for verification
        ...
        console.log(`   📊 Gallery items before this upload: ${countBeforeThisUpload}\n`);
        ...
        if (clipboardSuccess) {
          ...
          // STEP 4: Verify upload by monitoring gallery count
          console.log(`   ⏳ Waiting for image to appear in gallery (3 seconds)...`);
          await this.page.waitForTimeout(3000);
          
          // Verify the gallery count changed (image was added)
          const afterClipboardUpload = await this.page.evaluate(() => {
            return document.querySelectorAll('[data-testid="virtuoso-item-list"] a[href]').length;
          });
          
          console.log(`   📊 Items in gallery after clipboard paste: ${afterClipboardUpload}`);
          if (afterClipboardUpload > countBeforeThisUpload) {
            console.log(`   ✅ Image confirmed in gallery (count: ${countBeforeThisUpload} → ${afterClipboardUpload})`);
          } else {
            console.warn(`   ⚠️  Gallery count did not increase - image may not have been added`);
          }
          console.log('');
  ...
  async enterPrompt(prompt) {
    ...
    try {
      ...
      try {
        ...
        // Step 6: Press Enter
        console.log('   ⌨️  [6] Pressing Enter to submit...');
      ...
      // WAIT FOR PROMPT PROCESSING (5 seconds for AI to understand the prompt)
      console.log('   ⏳ Waiting 5 seconds for prompt to be processed...');
    ...
    try {
      ...
      for (let i = 0; i < prompts.length; i++) {
        ...
        try {
          ...
          console.log('[STEP B] ✅ Generate button clicked');
          console.log('[DELAY] ⏳ Waiting 2 seconds...');
          await this.page.waitForTimeout(2000);
          console.log('[DELAY] ✅ Ready');
          ...
          while (!generationDetected && monitoringAttempt < maxMonitoringAttempts) {
            ...
            // Check and retry any failed items (continuous monitoring)
            const wasRetried = await this.checkAndRetryFailedItemOnce();
            if (wasRetried) {
              failureRetryCount++;
              console.log(`   [FAILURES] ⚠️  Failed item was retried (attempt ${failureRetryCount}/${maxFailureRetries})`);
              
              // Prevent infinite loops - if we've retried too many times, fail this prompt
              if (failureRetryCount >= maxFailureRetries) {
                throw new Error(`[FAILURES] ❌ Generation failed after ${maxFailureRetries} retry attempts for prompt. Moving to next.`);
              }
              
              console.log(`   [FAILURES]    Continuing monitoring...`);
            }
