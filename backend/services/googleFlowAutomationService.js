...
class GoogleFlowAutomationService {
  ...
  /**
   * Generate single video with reference image and prompt
   * Used for multi-video generation workflows
   */
  async generateSingleVideo(prompt, options = {}) {
    const {
      imageBase64 = null,
      quality = 'high',
      aspectRatio = '9:16',
    ...
    try {
      ...
      // Configure settings (select Video tab, aspect ratio, etc.)
      console.log('⚙️  Configuring settings...');
      await this.configureSettings();
      console.log('✅ Settings configured\n');

      // Upload reference image if provided (as base64)
      if (imageBase64) {
        console.log('📤 Uploading reference image...');
        
        // Write base64 to temporary file
        const tempDir = path.join(process.cwd(), 'temp', 'video-ref-images');
        if (!fs.existsSync(tempDir)) {
          fs.mkdirSync(tempDir, { recursive: true });
        }

        const tempImagePath = path.join(tempDir, `ref-${Date.now()}.jpg`);
        const buffer = Buffer.from(imageBase64.split(',')[1] || imageBase64, 'base64');
        fs.writeFileSync(tempImagePath, buffer);

        // Upload the image by creating dummy paths
        // Since we only need 1 image for video, use it as both character and product
        try {
          await this.uploadImages(tempImagePath, tempImagePath, []);
          console.log('✅ Reference image uploaded\n');
          
          // Clean up temp file
          try { fs.unlinkSync(tempImagePath); } catch (e) {}
        } catch (uploadError) {
          console.warn(`⚠️  Reference image upload failed: ${uploadError.message}`);
      ...
      // Click video ingredients button if not already set
      if (this.type === 'video') {
        console.log('🎬 Clicking ingredients button for video...');
        try {
          await this.clickIngredientsButton();
          console.log('✓ Ingredients button clicked\n');
        } catch (e) {
          console.warn(`⚠️  Ingredients button error: ${e.message}, continuing...\n`);
        }
      }

      // Configure settings again to finalize
      console.log('⚙️  Finalizing settings...');
      await this.configureSettings();
      ...
      // Monitor generation - wait for video to appear in collection
      console.log('⏳ Waiting for video generation to complete (max 180s)...');
      
      let generatedVideoUrl = null;
      let previousUrl = null;

      // Wait for generation and NEW video to appear
      await this.page.waitForFunction(
        async () => {
          const currentUrl = await this.page.evaluate(() => {
            const video = document.querySelector('[data-testid="virtuoso-item-list"] [data-index="1"] video');
            const img = document.querySelector('[data-testid="virtuoso-item-list"] [data-index="1"] img');
            const elem = video || img;
            return elem && elem.src ? elem.src : null;
          });

          // If we have a URL and it's different from before, generation is complete
          if (currentUrl && (!previousUrl || currentUrl !== previousUrl)) {
            generatedVideoUrl = currentUrl;
        ...
        { timeout: this.options.timeouts.generation + 60000 } // Video takes longer
      );
