...
cloudGalleryController.getMediaByType = async (req, res) => {
  try {
    ...
    res.json({ success: true, type, count: media.length, data: media });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
...
cloudGalleryController.uploadMedia = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Media uploaded successfully', data: result });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
...
cloudGalleryController.downloadMedia = async (req, res) => {
  try {
    ...
    const result = await mediaManager.downloadMedia(fileId, outputDir || './downloads');
    res.json({ success: true, message: 'Media downloaded successfully', data: result });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
...
cloudGalleryController.organizeByCollection = async (req, res) => {
  try {
    ...
    res.json({ success: true, collectionCount: Object.keys(collections).length, data: collections });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
...
cloudGalleryController.manageBatchMedia = async (req, res) => {
  try {
    ...
    if (!batchId || !action || !mediaIds) {
      return res.status(400).json({ success: false, error: 'batchId, action, and mediaIds are required' });
    }
    const result = action === 'list' 
      ? await mediaManager.manageBatchMedia(batchId, action, [])
      : await mediaManager.manageBatchMedia(batchId, action, mediaIds);
    res.json({ success: true, message: `Batch media ${action} completed`, data: result });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
